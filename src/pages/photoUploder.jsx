...
export default function PhotoUploader({ uploadedPhoto, onChange }) {
  ...
  function removePhoto(filename) {
    onChange([...uploadedPhoto.filter((photo) => photo !== filename)]);
  }

  function selectAsMainPhoto(filename){
     removePhoto(filename)
     onChange([filename,...uploadedPhoto.filter((photo) => photo !== filename)])
  }
          ...
          uploadedPhoto.map((link) => (
            <div className="flex h-32 relative" key={link}>
              ...
              <div
                ...
                className="cursor-pointer absolute bottom-1 right-2 text-white bg-black bg-opacity-40 rounded-xl p-1"
              >
                <svg
              ...
              <div onClick={()=>{selectAsMainPhoto(link)}} className="cursor-pointer absolute bottom-1 left-2 text-white bg-black bg-opacity-40 rounded-xl p-1">
                {link !==uploadedPhoto[0] && (<svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={1.5}
